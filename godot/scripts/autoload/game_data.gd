extends Node
## GameData autoload — ports game data and constants from shared.js
...
# --- Shop Odds (level -> array of 5 percentages for tiers 1-5) ---
...
## Roll a random shop unit based on player level and shop odds.
## Returns a unit_id string.
...
## Calculate hex distance between two hex keys (format "col,row").
func hex_distance(key1: String, key2: String) -> int:
	var parts1 := key1.split(",")
	var parts2 := key2.split(",")
	var col1 := int(parts1[0])
	var row1 := int(parts1[1])
	var col2 := int(parts2[0])
	var row2 := int(parts2[1])

	# Convert offset coordinates to cube coordinates
	var x1 := col1 - int((row1 - (row1 & 1)) / 2)
	var z1 := row1
	var y1 := -x1 - z1

	var x2 := col2 - int((row2 - (row2 & 1)) / 2)
	var z2 := row2
	var y2 := -x2 - z2

	return int((absi(x1 - x2) + absi(y1 - y2) + absi(z1 - z2)) / 2)


## Get the sell value for a unit given its cost and star level.
...
## Get active synergy bonuses for a board (Dictionary of hex_key -> unit dict).
## Returns a Dictionary of faction_name -> bonus Dictionary.
func get_synergy_bonuses(board: Dictionary) -> Dictionary:
	...
	for hex_key in board:
		...
		if unit == null:
			continue
		var faction: String = ""
		if unit is Dictionary and unit.has("faction"):
			faction = unit["faction"]
		elif unit is Dictionary and unit.has("id"):
			var unit_def = units_data.get(unit["id"], {})
			faction = unit_def.get("faction", "")
		if faction != "":
...
## Create a combat-ready unit dictionary from a unit_id.
## star_level: 1, 2, or 3
## synergies: result of get_synergy_bonuses() (only applied for player units)
func create_combat_unit(unit_id: String, hex_key: String, is_player: bool, synergies: Dictionary = {}, stars: int = 1) -> Dictionary:
	...
	# Apply synergy bonuses (player units only)
	...
	var combat_unit: Dictionary = {
		...
		"last_move_time": 0.0,
		"action_cooldown": 1.0 / base_stats["speed"],
		"move_cooldown": 0.5 / base_stats["speed"],
		"status_effects": [],
		"has_cast": false,
		"has_revived": false,
