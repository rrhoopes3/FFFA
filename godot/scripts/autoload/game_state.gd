...
var shop_units: Array = []  # Array of unit_id strings (or null for empty slots)
var player_board: Dictionary = {}  # "col,row" -> {id: String, stars: int}
var enemy_board: Dictionary = {}
var bench: Array = []  # Array of 9 items, each null or {id: String, stars: int}
...
var drag_source: String = ""  # "board", "bench", "shop"
var drag_source_key: String = ""  # hex key or bench index
var highlight_hex = null

# Player management for 8-player mode
var mode: String = "single"  # single, multiplayer, online
var players: Array = []  # Array of player state dicts
...
func try_buy_unit(shop_index: int) -> bool:
	...
	if unit_id.is_empty():
		return false
	var unit_data = GameData.units_data.get(unit_id)
	if not unit_data:
		return false
	...
	shop_units[shop_index] = ""  # Mark as bought
	EventBus.gold_changed.emit(gold)
...
func sell_unit_from_bench(bench_index: int) -> bool:
	...
	if unit == null:
		return false
	var unit_data = GameData.units_data.get(unit.id)
	if not unit_data:
		return false
	var sell_value := GameData.get_sell_value(unit_data.cost, unit.stars)
	gold += sell_value
...
func sell_unit_from_board(hex_key: String) -> bool:
	...
	if not unit_data:
		return false
	var sell_value := GameData.get_sell_value(unit_data.cost, stars)
	gold += sell_value
